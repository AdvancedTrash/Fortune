...
--Revamped by DeviousQuacks23, 2025 (happy 10th anniversary?)
...
function autoscroll.onTick()
	...
	if shouldDiscardOnCameraDraw then
		unregisterEvent(autoscroll, "onCameraDraw")
...
function autoscroll.lockScreen(playerIdx)
	...
	local x2, y2 = x1 + camera.width, y1 + camera.height -- Incompatible with split-screen
	setSectionBounds(section, x1, y1, y2, x2)
	active_scrolls[section] = nil
end
