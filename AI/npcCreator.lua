...
local function playSound(v)
	...
	if data.ignoreDeathSound then return end
...
local function spawnNPC(v)
	...
	for i = 1,data.burst[data.state + 1] do
		...
		if v.heldIndex ~= 0 then
			n.heldIndex = Player.getNearest(v.x + v.width/2, v.y + v.height).idx
			Player.getNearest(v.x + v.width/2, v.y + v.height):mem(0x154, FIELD_WORD, n.idx+1)
		end
...
local function init(v)
	...
	data.horizontalMovementSpeedTemp = data.horizontalMovementSpeed[data.state + 1]
...
local function changeStates(v)
	...
	data.saveSpeed = vector.v2(data.horizontalMovementSpeed[data.state + 1], data.verticalMovementSpeed[data.state + 1])
	if data.saveSpeed.y == 0 then data.saveSpeed.y = 0.001 end
	data.slowDown = true
	...
	if data.phaseDirection[data.state + 1] == 4 then v.direction = -v.direction return end
	if data.phaseDirection[data.state + 1] == 5 then return end
...
function sampleNPC.onTickNPC(v)
	...
	if (data.horizontalMovementSpeed[data.state + 1] == 0 or v.dontMove) and not data.slowDown and not v.isProjectile then v.speedX = 0 end
	...
	then
		...
		data.spawntimer = 0
		data.meleetimer = 0
		data.stateVariable = data.grab[data.state + 1]				
		changeStates(v)
		...
		if not data.activeWaypoint then
			...
			if not data.flipped[data.state + 1] then
				if data.generalMovement[data.state + 1] == 0 then
					if data.horizontalMovement[data.state + 1] == 0 then
						...
						if not data.bounceAnimation or data.bounceAnimation <= 1 then
							if not data.speedAdjust[data.state + 1] or not data.saveSpeed then
								v.speedX = (data.horizontalMovementSpeed[data.state + 1] * data.speedDeduction) * v.direction
								data.slowDown = nil
							else
								if math.abs(v.speedX) <= data.saveSpeed.x then
									data.slowDown = nil
									v.speedX = math.clamp(v.speedX + (0.125 * data.speedDeduction) * math.sign(data.saveSpeed.x) * v.direction, -data.saveSpeed.x, data.saveSpeed.x)
								else
									v.speedX = v.speedX - ((0.125 * data.speedDeduction) * v.direction)
								end
							end
						else
							data.npcharm[data.state + 1] = true
						end
					...
					if not data.gravity[data.state + 1] then
						if data.verticalMovement[data.state + 1] == 0 then
							if not data.speedAdjust[data.state + 1] or not data.saveSpeed then
								...
								if math.abs(v.speedY - data.saveSpeed.y) <= 1 then
									v.speedY = data.verticalMovementSpeed[data.state + 1] * data.speedDeduction
								else
									v.speedY = v.speedY - ((0.125 * data.speedDeduction) * -math.sign(data.saveSpeed.y))
								end
							end
			...
			--Move on redirectors
			if data.redirector[data.state + 1] and (not data.bounceAnimation or data.bounceAnimation < 2) then
				for _,bgo in ipairs(BGO.getIntersecting(v.x+(v.width/2)-0.5,v.y+(v.height/2),v.x+(v.width/2)+0.5,v.y+(v.height/2)+0.5)) do
					if redirector.VECTORS[bgo.id] then -- If this is a redirector and has a speed associated with it
						local redirectorSpeed = redirector.VECTORS[bgo.id] * data.horizontalMovementSpeedTemp -- Get the redirector's speed and make it match the speed in the NPC's settings		
		...
		elseif data.activeWaypoint == 2 then
			...
			if not data.teleport then
				...
				if data.noHurt <= 0 then data.noHurt = 24 end
	...
	for _, p in ipairs(data.NPCHarm) do
		if p.idx ~= v.idx then
			for _,e in ipairs(NPC.config[v.id].coolListOfThings) do
				...
				if not data.gravity[data.state + 1] or NPC.config[p.id].iscoin or NPC.config[p.id].isvine or NPC.config[p.id].noblockcollision or p.isHidden or p.friendly or p.despawnTimer <= 0 or p.heldIndex ~= 0 or NPC.config[p.id].ignorethrownnpcs or p.collisionGroup ~= v.collisionGroup then
					data.noTurn = 4
				end
	...
	if data.rotationType[data.state + 1] > 0 then
		if data.rotationType[data.state + 1] == 2 then
			...
			if data.rotationType[data.state + 1] == 6 then
...
function sampleNPC.onNPCHarm(eventObj, v, reason, culprit)
	...
	if data.harmType then
		if reason == data.harmType[1] and data.harmType2[1] == 1 and data.hurtfulStomp[data.state + 1] or (reason == data.harmType[8] and data.harmType2[8] == 1 and data.harmType2[1] == 0) then
			...
			--Act like a spring
			if data.spring[data.state + 1] > 0 and (reason == HARM_TYPE_JUMP or reason == HARM_TYPE_SPINJUMP) then
				eventObj.cancelled = true
				data.playerFriendly2 = true
				SFX.play(24)
				if culprit.keys.jump == KEYS_DOWN or culprit.keys.altJump == KEYS_DOWN then culprit.speedY = -data.springForce[data.state + 1] else culprit.speedY = 0.5 * -data.springForce[data.state + 1] end
				if data.spring[data.state + 1] == 1 then
					data.bounceAnimation = 1 data.bounceAnimationTimer = vector.zero2
				end
				data.stateVariable = data.jump[data.state + 1]				
				changeStates(v)
				return
	...
	if not data.kill then
		...
		if (not data.hurtfulStomp[data.state + 1] or data.spring[data.state + 1] > 0) and reason == HARM_TYPE_JUMP then
			eventObj.cancelled = true
			return
		end
	end
