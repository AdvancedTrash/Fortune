...
local timer1 = minTimer.create{initValue = minTimer.toTicks{hrs = 0, mins = 3, secs = 0}, x = 150, y = 552}
...
local bossMaxHP = bossMaxHP or 45 -- Set in NPC Creator too, couldn't pull the HP data directly for some reason
local bossCurrentHP = bossCurrentHP or 45 -- Set in NPC Creator too, couldn't pull the HP data directly for some reason
...
function onDraw()
    ...
    if bossChallengeOff == false then
        ...
        textplus.print{
                text = string.format("Kore Rematch: Hard Mode"),
                font = minFont,
                priority = 5,
                wave = 1,
                pivot = Sprite.align.TOP,
                ...
                xscale = 2, yscale = 2,
                color = Color.fromHexRGBA(0xFF0000FF) * myOpacity
        }
...
function onEvent(eventName)
    if eventName == "boss" and Layer.get("BossWin").isHidden then
        timer1:start()
        bossChallengeOff = false
        emergencyClose = false
        myOpacityChange = 0.05
    elseif eventName == "BossWin" and minTimer.activeTimer.id == timer1.id then
        timer1:close(minTimer.WIN_CLEAR, true)
        Layer.get("BossWin"):show(true)
        bossChallengeOff = true
        myOpacityChange = -0.05
        emergencyClose = false
    end
