	...
	if not npc.isGenerator and npc.data._basegame then
		...
		if data.state == STATE_FULL and p then
			...
			if player.character == CHARACTER_KLONOA then
				...
				npc.ai3 = npc.ai3 + 1;
					if npc.ai3 >= NPC.config[960].second then
						if data.memory > 0 then
...
function launchBarrel.boo.onTickNPC(npc)
	...
	if settings.visTime == nil then
		settings.visTime = 48
		settings.invisTime = 48
	end
	...
	if data.isVisible then
		if data.opacity >= 1 then
			...
			if data.opacity > 0.75 then
				npc.friendly = false
			end
		end
		if data.fadeTimer >= settings.visTime then
			data.fadeTimer = 0
		...
		if data.fadeTimer >= settings.invisTime then
			data.fadeTimer = 0
			data.isVisible = true
		end
	end
...
function launchBarrel.boost.onTickNPC(npc)
	utils.applyLayerMovement(npc)
	if Defines.levelFreeze or npc:mem(0x12A, FIELD_WORD) <= 0 then return end
	...
	if data.boosterSlowingDown == nil then
		data.boosterSlowingDown = 1
		data.boosterSpeed = vector.zero2
		data.canMoveBooster = true
	end
	...
	elseif data.state == STATE_FULL then
		...
		--When it touches a terminus bgo, start slowing down
		for _,bgo in ipairs(BGO.getIntersecting(npc.x+(npc.width/2)-0.5,npc.y+(npc.height/2),npc.x+(npc.width/2)+0.5,npc.y+(npc.height/2)+0.5)) do
			if bgo.id == redirector.TERMINUS then -- If this BGO is one of the crosses
				-- Simply make the NPC stop moving
				data.boosterSlowingDown = 0
			end
		end
		
		--Effect.spawn(10, npc.x + npc.width * 0.25, npc.y + npc.height * 0.25)
		
		if data.canMoveBooster then
			if data.memory <= 0 and settings.counttimer > 0 then
				npc.speedX = 0
				npc.speedY = 0
				data.boosterSlowingDown = 1
				data.boosterSpeed = vector.zero2
				exitBarrel(npc, data.player)
				data.canMoveBooster = false
			end
			if data.boosterSlowingDown == 0 then
				npc.speedX = npc.speedX * 0.92
				npc.speedY = npc.speedY * 0.92
				if (math.abs(npc.speedX) < 0.1 and math.abs(npc.speedY) < 0.1) then
					npc.speedX = 0
					npc.speedY = 0
					data.boosterSlowingDown = 1
					data.boosterSpeed = vector.zero2
				...
				if lunatime.tick() % 4 == 0 then
					data.w = pi/180
					Effect.spawn(131, npc.x + (npc.width * 0.25 + 2200 * -data.w * cos(data.w*data.rotation - RNG.random(7.5, 8.2))), npc.y + npc.height * 0.25 + 2200 * -data.w * sin(data.w*data.rotation - RNG.random(7.5, 8.2)))
				end
				if data.boosterSlowingDown == 3 then
					npc.speedX = 4 * cos(ang)
					npc.speedY = 4 * sin(ang)
				else
					if (math.abs(data.boosterSpeed.x) < 4 or math.abs(data.boosterSpeed.y) < 4) then
						data.boosterSpeed.x = data.boosterSpeed.x + 0.1
						data.boosterSpeed.y = data.boosterSpeed.y + 0.1
					
						npc.speedX = data.boosterSpeed.x * cos(ang)
						npc.speedY = data.boosterSpeed.y * sin(ang)
					end
				end
			end
		else
		...
		--Forget this code exists it's shit
		if (npc.y < npc.spawnY and ((data.rotation >= 271 and data.rotation <= 360) or (data.rotation >= 0 and data.rotation <= 89))) or (npc.y > npc.spawnY and (data.rotation >= 90 and data.rotation <= 270)) or ((data.rotation == 90 and npc.x > npc.spawnX) or (data.rotation == 270 and npc.x < npc.spawnX)) then
			data.boosterSlowingDown = 3
			if (npc.y > npc.spawnY - 48 and ((data.rotation >= 271 and data.rotation <= 360) or (data.rotation >= 0 and data.rotation <= 89))) or (npc.y < npc.spawnY - 48 and (data.rotation >= 90 and data.rotation <= 270)) or ((data.rotation == 90 and npc.x < npc.spawnX - 48) or (data.rotation == 270 and npc.x > npc.spawnX - 48)) then
				data.boosterSpeed.x = data.boosterSpeed.x - 0.1
				data.boosterSpeed.y = data.boosterSpeed.y - 0.1
...
function launchBarrel.track.onTickNPC(npc)
	utils.applyLayerMovement(npc)
	if Defines.levelFreeze or npc:mem(0x12A, FIELD_WORD) <= 0 then return end
	...
	if data.trackerChase == nil then
		if settings.trackerTrueTrack then
			data.trackerChase = false
		else
			data.trackerChase = true
		end
	end
	...
	elseif data.state == STATE_FULL then
		...
		local plr = Player.getNearest(npc.x + npc.width / 2, npc.y + npc.height / 2)
	
		if settings.trackerTrueTrack and data.trackerChase then
			if (plr.speedY == 0 or plr:mem(0x176,FIELD_WORD) ~= 0 or plr:mem(0x48,FIELD_WORD) ~= 0) then
				data.trackerChase = false
			end
		end
		
		
		if not data.trackerChase then
			npc.speedX = 0
			npc.speedY = 0
		else
			if settings.trackerDir == 0 then
				if math.abs((plr.x - 16) - npc.x) <= 16 then
					npc.speedX = 0
				else
					if not (npc.collidesBlockRight or npc.collidesBlockLeft) then
						if plr.x < npc.x then
							npc.speedX = -settings.trackerSpeed
						else
							npc.speedX = settings.trackerSpeed
						end
					else
				...
				if math.abs((plr.y - 12) - npc.y) <= 12 then
					npc.speedY = 0
				else
					if not (npc.collidesBlockUp or npc.collidesBlockBottom) then
						if plr.y < npc.y then
							npc.speedY = -settings.trackerSpeed
						else
							npc.speedY = settings.trackerSpeed
						end
					else
		...
		npc.config[npc.id].noblockcollision = false
		data.flipTimer = data.flipTimer + 1
		data.rotation = data.rotation + settings.rotSpeed*npc.direction*data.flipDirection
		checkPlayersForBarrel(npc)
		updateRotation(npc)
...
function launchBarrel.steer.onTickNPC(npc)
	local settings = npc.data._settings
	utils.applyLayerMovement(npc)
	if Defines.levelFreeze or npc:mem(0x12A, FIELD_WORD) <= 0 then return end
	...
	if data.cooldown > 0 then
		...
		npc.speedX = 0
		npc.speedY = 0
		npc.config[npc.id].noblockcollision = true
	elseif data.state == STATE_FULL then
		local p = data.player
		p.speedX = 0
		p.speedY = 0
		...
		elseif p.keys.jump == KEYS_PRESSED or data.memory <= 0 and settings.counttimer > 0 then
			...
			npc.x = npc.spawnX
			npc.y = npc.spawnY
			Effect.spawn(760,npc.x,npc.y)
		elseif p.keys.up then
			dir.y = -npc.config[npc.id].movementSteerSpeed
		elseif p.keys.down then
			dir.y = npc.config[npc.id].movementSteerSpeed
		end
		if p.keys.left then
			dir.x = -npc.config[npc.id].movementSteerSpeed
		...
		if not p.keys.left and not p.keys.right and not p.keys.up and not p.keys.down then
			...
			npc.speedX = npc.speedX * 0.95
			if math.abs(npc.speedX) < 0.1 then
				npc.speedX = 0
			end
			
			npc.speedY = npc.speedY * 0.95
			if math.abs(npc.speedY) < 0.1 then
				npc.speedY = 0
			end
		...
		for _,b in ipairs(Block.getIntersecting(npc.x - 1, npc.y - 1, npc.x + npc.width + 1, npc.y + npc.height + 1)) do
			if p.deathTimer == 0 and not Defines.cheat_donthurtme then
				if Block.LAVA_MAP[b.id] or b.id == 673 then
					p:kill()
				elseif Block.HURT_MAP[b.id] then
					p:harm()
				end
			end
		end
		
		for k, v in ipairs(NPC.getIntersecting(npc.x - 1, npc.y - 1, npc.x + npc.width + 1, npc.y + npc.height + 1)) do
			if not (v.friendly and v.isGenerator and not v.isHidden) and not (NPC.config[v.id].nohurt or NPC.config[v.id].isinteractable or NPC.config[v.id].iscoin) then
				p:harm()
			elseif NPC.config[v.id].isinteractable or NPC.config[v.id].iscoin then
				v.x = p.x
				v.y = p.y
			end
		end
