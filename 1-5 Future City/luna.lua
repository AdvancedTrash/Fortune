...
function onDraw()
    ...
    if timerChallengeOff == false then
        ...
        textplus.print{
                text = string.format("Launcher Challenge: Dodge and survive!"),
                font = minFont,
                priority = 5,
                wave = 1,
                pivot = Sprite.align.TOP,
...
function onEvent(eventName)
    ...
    --Lakitu Challenge
    if eventName == "WallChallenge" then
        timer2:start()
        timerChallengeOff = false
        emergencyClose = false
        myOpacityChange = 0.05
        
    elseif eventName == "WallChallengeWin" and minTimer.activeTimer.id == timer2.id then
