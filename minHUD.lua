...
local luigiCoinEmpty = Graphics.loadImageResolved("LuigiUncollected.png")
local luigiCoinFull = Graphics.loadImageResolved("LuigiCollected.png")
...
function minHUD.drawHUD(camIdx,priority,isSplit)
    ...
    if coinCounterSet == 1 then
        if iconOri == 2 then
            Graphics.drawImageWP(coinCounter, 432, 4, priority)
            textplus.print{
                text = string.format(coinCountZeros[coinCountZD], SaveData.customCoins or 0),
                font = minFont,
                priority = 5,
                x = 466, y = 4,
                xscale = 2, yscale = 2,
    ...
    if scoreCounterSet == 1 then
        if iconOri == 2 then
            textplus.print{text = string.format(scoreCountZeros[scoreZD], SaveData._basegame.hud.score), font = minFont, priority = 5, x = 550, y = 4, xscale = 2, yscale = 2, color = Color.fromHexRGBA(0xFFFFFFFF)}
        else
            textplus.print{text = string.format(scoreCountZeros[scoreZD], SaveData._basegame.hud.score), font = minFont, priority = 5, x = 560, y = 4, xscale = 2, yscale = 2, color = Color.fromHexRGBA(0xFFFFFFFF)}
        end
    end
    ...
    if livesAltStyle == 2 then
        if iconOri == 2 then
            Graphics.drawImageWP(deathCounter, 286, 4, priority)
            ...
            Graphics.drawImageWP(deathCounter, 146, 4, priority)
    ...
    if starCounterSet == 1 then
        if iconOri == 2 then
            Graphics.drawImageWP(starCounter, 28, 4, priority)
            textplus.print{text = string.format(starsCountZeros[starsZD],mem(0x00B251E0, FIELD_WORD)), font = minFont, priority = 5, x = 62, y = 4, xscale = 2, yscale = 2, color = Color.fromHexRGBA(0xFFFFFFFF)}
        else
    ...
    --Luigi Coins
    local luigiIconToDraw = luigiCoinEmpty
    local levelName = Level.filename()

    if SaveData.checklist[levelName] then
        local data = SaveData.checklist[levelName]
        if data.luigiCoinInLevel then
            luigiIconToDraw = data.luigiCollected and luigiCoinFull or luigiCoinEmpty
             if iconOri == 2 then
                Graphics.drawImageWP(luigiIconToDraw, 132, 4, priority)
             else 
                Graphics.drawImageWP(luigiIconToDraw, 132, 4, priority)
            end
        end
    ...
    if timeCounterSet == 1 then
        if iconOri == 2 then
            if timeAltStyle == 2 then
                Graphics.drawImageWP(timeCounterB, 682, 4, priority)
            else
                Graphics.drawImageWP(timeCounter, 682, 4, priority)
            end
            textplus.print{text = tostring(Timer.getValue()), font = minFont, priority = 5, x = 716, y = 4, xscale = 2, yscale = 2, color = Color.fromHexRGBA(0xFFFFFFFF)}
        else
            if timeAltStyle == 2 then
                Graphics.drawImageWP(timeCounterB, 704, 4, priority)
            else
                Graphics.drawImageWP(timeCounter, 704, 4, priority)
            end
            textplus.print{text = tostring(Timer.getValue()), font = minFont, priority = 5, x = 734, y = 4, xscale = 2, yscale = 2, color = Color.fromHexRGBA(0xFFFFFFFF)}
        end
    end    
