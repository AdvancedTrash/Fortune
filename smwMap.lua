...
local dragonCoinUncollectedImage = Graphics.loadImageResolved("dragonCoinEmptyW.png")
local dragonCoinCollectedImage   = Graphics.loadImageResolved("dragonCoinCollect.png")
local luigiCoinCollectedImage = Graphics.loadImageResolved("LuigiCollected.png")
local luigiCoinUncollectedImage = Graphics.loadImageResolved("LuigiUncollected.png")
local starCollectedImage = Graphics.loadImageResolved("StarCollected.png")
local starUncollectedImage = Graphics.loadImageResolved("StarUncollected.png")
local miniGameComplete = Graphics.loadImageResolved("MiniGameMastered.png")
local miniGameWon = Graphics.loadImageResolved("MiniGame.png")
local miniGameUnfinished = Graphics.loadImageResolved("MiniGameUnfinished.png")
...
do
    ...
    smwMap.hudCounters = {
        ...
        -- Coins
        {
            ...
            getValue = function()
                return SaveData.customCoins or 0
            end,
        },

        -- Star Coins
        {
            icon = Graphics.loadImageResolved("dragonCoinCollect.png"),
            isEnabled = function()
                return (starcoin.getEpisodeCollected() or 0) > 0
            end,
            getValue = function()
                return starcoin.getEpisodeCollected() or 0
            end,
        },
        ...
        -- Luigi Coins
        {
            icon = Graphics.loadImageResolved("luigicoin.png"),
            isEnabled = function()
                return checklist.getLuigiCoinTotal() > 0
            end,
            getValue = function()
                return checklist.getLuigiCoinTotal() 
            end,
        },

        -- Deaths
        {
            icon = Graphics.loadImageResolved("deathCounter.png"),
            isEnabled = function()
                return (SaveData.deathCount or 0) > 0
            end,
            getValue = function()
                return SaveData.deathCount or 0
            end,
        },
    }
    ...
    function smwMap.drawHUD()
        ...
        if hudSettings.starcoinsEnabled and levelObj ~= nil then
            ...
            if starcoinCount ~= nil and starcoinCount > 0 then
                ...
                local uncollectedImage = dragonCoinUncollectedImage
                local collectedImage = dragonCoinCollectedImage
        ...
        if smwMap and Level.filename() == smwMap.levelFilename then
            if smwMap.mainPlayer and smwMap.mainPlayer.levelObj then
                curFilename = smwMap.mainPlayer.levelObj.settings.levelFilename
            end

        elseif isOverworld and world.levelObj then
            curFilename = world.levelObj.filename
        else
            curFilename = Level.filename()
        end

        if not curFilename then return end

        local data = SaveData.checklist[curFilename]
        ...
        -- Luigi Coin
        if data.luigiCoinInLevel then
            local icon = data.luigiCollected and luigiCoinCollectedImage or luigiCoinUncollectedImage
             Graphics.drawImageWP(icon, iconX, iconY, 10)
        end

        -- Star
        if data.starInLevel then
            local starIcon = data.starCollected and starCollectedImage or starUncollectedImage
            Graphics.drawImageWP(starIcon, stariconX, stariconY, 10)
        end

        -- Mini Game and Score
        if data.miniGameInLevel then
            local miniIcon = data.miniGameMastered and miniGameComplete or data.miniGameWon and miniGameWon or miniGameUnfinished
            Graphics.drawImageWP(miniIcon, miniGameX, miniGameY, 10)
            textplus.print{
                text = string.format("%d", data.miniGameHighScore or 0),
                font = minFont,
                priority = 5,
                x = 544, y = 550,
                xscale = 2, yscale = 2,
                color = Color.fromHexRGBA(0xFFFFFFFF)
        }
        end
...
smwMap.hudSettings = {
    ...
    levelTitleFont = textplus.loadFont("minFont.ini"),
    levelTitleColor = Color.white,
    levelTitleshadowColor = Color.black,
    levelTitleshadowOffset = vector(1,1),
...
smwMap.selectStartPointSettings = {
    ...
    textFont = textplus.loadFont("minFont.ini"),
    textScale = 2,
