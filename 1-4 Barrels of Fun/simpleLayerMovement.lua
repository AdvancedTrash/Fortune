...
function slm.onInitAPI()
    ...
    registerEvent(slm,"onPlayerKill")
end
...
function slm.onTickEnd()
    ...
    if Defines.levelFreeze or Layer.isPaused() or Misc.isPaused() then return end
    ...
    for k,v in ipairs(slm.movementObjs) do
        ...
        if not Defines.levelFreeze and not Layer.isPaused() and not Misc.isPaused() then
            v.timer = v.timer + 1
        end
